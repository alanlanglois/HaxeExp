...
@:dox(hide) class ApplicationMain {
	...
	public static function main () {
		...
		config = {
			...
			name: "First",
			orientation: "",
			packageName: "com.etamin.first.First",
			version: "1.0.0",
			windows: [
	...
	macro public static function getEntryPoint () {
		...
		switch (Context.follow (Context.getType ("com.etamin.first.Main"))) {
			...
			case TInst (t, params):
				...
				if (hasMain) {
					...
					return Context.parse ("@:privateAccess com.etamin.first.Main.main ()", Context.currentPos ());
					...
					Context.fatalError ("Main class \"com.etamin.first.Main\" has neither a static main nor a constructor.", Context.currentPos ());
					
				}
				
			default:
				
				Context.fatalError ("Main class \"com.etamin.first.Main\" isn't a class.", Context.currentPos ());
...
@:keep @:dox(hide) class DocumentClass extends com.etamin.first.Main {}
